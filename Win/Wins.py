...
class ArtLsWindow(wx.Frame):
    def __init__(self, AboutWindow, title):
        ...
        Sequence = self.tree.AppendItem(root, "[Sequence]")
        self.tree.Bind(wx.EVT_TREE_SEL_CHANGED, self.on_item_selected)
    ...
    def on_item_selected(self, event):
        ...
        elif itemtext == "[Sequence]":
            self.SetStatusText(" [Sequence]，步兵的动作序列")
        else:
            self.SetStatusText("")
...
class SoundLsWindow(wx.Frame):
    ...
    def on_item_selected(self, event):
        ...
        itemtext = self.tree.GetItemText(item)
        if itemtext == "[Sound]":
            self.SetStatusText(" [Sound]，声音/语音/音效设定")
        else:
            self.SetStatusText("")
...
class INIFilesWindow(wx.Frame):
    def __init__(self, AboutWindow, title):
        ...
        MentalOmegaMaps = self.tree.AppendItem(root, "MentalOmegaMaps")
        Map = self.tree.AppendItem(root, "map ✚")
        self.tree.AppendItem(Map, "map")
        self.tree.AppendItem(Map, "yrm")
        Snow = self.tree.AppendItem(root, "snow")
        Movie = self.tree.AppendItem(root, "movie")
        Battle = self.tree.AppendItem(root, "battle")
        Mission = self.tree.AppendItem(root, "mission")
        Ra2 = self.tree.AppendItem(root, "ra2 ✚")
        self.tree.AppendItem(Ra2, "RA2.ini")
        self.tree.AppendItem(Ra2, "RA2MD.ini")
        self.tree.AppendItem(Ra2, "RA2MO.ini")
        Rgb = self.tree.AppendItem(root, "rgb")
    ...
    def on_item_selected(self, event):
        ...
        elif itemtext == "MentalOmegaMaps":
            self.SetStatusText(" MentalOmegaMaps.ini，心灵终结-多人地图属性")
        elif itemtext == "map ":
            self.SetStatusText(" map，地图内置ini")
        elif itemtext == "ra2 ":
            self.SetStatusText(" ra2，一些基础控制")
        elif itemtext == "snow":
            self.SetStatusText(" snow，雪地地图控制")
        elif itemtext == "movie":
            self.SetStatusText(" movie，影片控制文件")
        elif itemtext == "battle":
            self.SetStatusText(" battle，战役、作战控制")
        elif itemtext == "mission":
            self.SetStatusText(" mission，战役控制")
        elif itemtext == "rgb":
            self.SetStatusText(" rgb，颜色(?)控制")
        else:
            self.SetStatusText("")
        ...
        elif childitemtext == "map":
            self.SetStatusText(" map，原版/心灵终结等的地图文件")
        elif childitemtext == "yrm":
            self.SetStatusText(" yrm，尤里的复仇等的地图文件")
        elif childitemtext == "RA2.ini":
            self.SetStatusText(" RA2.ini，原版的一些基础控制文件")
        elif childitemtext == "RA2MD.ini":
            self.SetStatusText(" RA2MD.ini，尤里的复仇的一些基础控制文件")
        elif childitemtext == "RA2MO.ini":
            self.SetStatusText(" RA2MO.ini，心灵终结的一些基础控制文件")
